...
_IMG_FORMAT = {
    ...
    'clipped_data': 'frame{:06d}{:s}.jpg',
}


def load_info(name, root, task='train', mode='rgb', split=1):
    if name in ['ucf101', 'hmdb51', 'clipped_data']:
...
def get_info(name, root, mode='rgb', split=1):
    base_dir = root
    data_dir = os.path.join('data', name, mode + '.csv')
    test_split = os.path.join('data', name, 'testlist%02d' % split + '.txt')
    print(data_dir)
    print(test_split)
    #data_dir = '/data1/yunfeng/i3d_test/data/'+name+'/'+mode+'.txt'
    #test_split = '/data1/yunfeng/i3d_test/data/'+name+'/testlist%02d' % split+'.txt'
    f_1 = open(data_dir, 'r')
    test = [x.split(os.path.sep)[1].split('.')[0]
