...
class ActionDataset(Dataset):
    ...
    def gen_test_list(self, sample=1, mode='rgb'):
        test_list = []
        name = []
        for i, video in enumerate(self.videos):
            if mode == 'flow':
                test_list.append(([i, 15, 0, sample, False, False],))
            for j in range(video.total_frame_num):
                test_list.append(([i, 15, j+1, sample, False, False],))
    ...
    def get_video(self, info):
        ...
        #print(info)
        #print(info[1])
        return video, np.array([label]), info
